...
import type { RoomState, Player, CategorySelection, ModelProvider } from '../types';
import { CATEGORY_OPTIONS, MODEL_PROVIDER_OPTIONS } from '../types';
    ...
    <div className="flex min-h-screen flex-col items-center bg-prompt-black p-4 md:p-8">
      ...
      <div className="mb-8 md:mb-10">
        <AvatarSelector
          ...
          aiAvatarsEnabled={room.modelProvider === 'nano-banana'}
        />
      </div>

      {/* Game Settings (Host Only) */}
      {isHost && (
        <div className="mb-8 w-full max-w-md space-y-4 md:max-w-lg md:space-y-6">
          {/* Category Selection */}
          <div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 md:gap-3">
              {MODEL_PROVIDER_OPTIONS.map((provider) => (
                <button
                  key={provider.id}
                  onClick={() => onSetModelProvider(provider.id)}
                  className={`rounded-lg border-2 p-3 text-left transition-all md:p-4 ${
                    room.modelProvider === provider.id
                      ? 'border-prompt-purple bg-prompt-purple/20'
                      : 'border-gray-700 bg-gray-900 hover:border-gray-600'
                  }`}
                >
                  <div className="font-semibold text-white">{provider.name}</div>
                  <div className="text-xs text-gray-400 md:text-sm">{provider.description}</div>
                </button>
              ))}
            </div>
          </div>
      ...
      {!isHost && (
        <div className="mb-8 flex flex-wrap justify-center gap-3 md:gap-4">
          <div className="rounded-lg border border-gray-700 bg-gray-900/50 px-4 py-2 text-center md:px-6 md:py-3">
          ...
          <div className="rounded-lg border border-gray-700 bg-gray-900/50 px-4 py-2 text-center md:px-6 md:py-3">
            <span className="text-xs text-gray-500 md:text-sm">Generator: </span>
            <span className="text-sm font-semibold text-prompt-purple md:text-base">
